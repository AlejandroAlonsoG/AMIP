...
class UNet2(nn.Module):
    def __init__(self, num_classes, class_mapping, input_channels=3):
        ...
        self.decoder5 = ConvBlock(2048, 1024)
    ...
    def forward(self, x):
        ...
        encoder_5_output = self.encoder5(encoder_4_pooling)
        encoder_5_pooling = self.pool5(encoder_5_output)

        bridge_output = self.bridge(encoder_5_pooling)

        decoder_5_upconv = self.upconv5(bridge_output)
        decoder_5_output = self.decoder5(torch.cat([decoder_5_upconv, encoder_5_output], dim=1))

        decoder_4_upconv = self.upconv4(decoder_5_output)
